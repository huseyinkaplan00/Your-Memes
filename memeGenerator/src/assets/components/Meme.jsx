...
export default function Meme() {
	...
	const handleMoreInputsChange = (event) => {
		...
		setExtraInput(extraInput.map((prevInput) => (prevInput.id === name ? { ...prevInput, value: value } : prevInput)))
	}
	...
	const gettingImages = () => {
		...
		const url = memes[randomNumber] ? memes[randomNumber].url : "/memeGenerator/placeHolderImage.svg"
		...
		<main>
			<div className="inputs">
				...
				<button
					...
					onClick={addInput}
					style={{ boxShadow: boxShadowColor ? `0px 0px 0px 2px ${boxShadowColor}` : "none" }}
				>
					<small>+</small>
				</button>
			</div>
			...
			<button
				style={{ boxShadow: boxShadowColor ? `-1px 0px 0px 2px ${boxShadowColor}` : "none" }}
			...
			>
				<img
					style={{ boxShadow: boxShadowColor ? `-1px 0px 3px 2px ${boxShadowColor}` : "none" }}
				...
				<Draggable bounds="parent">
					<h3
						style={style}
						className="main--content__image--bottomText"
					>
						{" "}
						{texts.bottomText}{" "}
					</h3>
